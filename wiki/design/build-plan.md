...
| 6 | **Required** (was optional) — the prediction layer: predicted muzzle flash / cast SFX / HUD drain, with a predicted-endpoint parameter. Stage 4 deletes the client branch that raises the caster's muzzle flash today, so this is needed to avoid a feel regression; and it is the prerequisite for any future hitscan skill. | Post-launch |
