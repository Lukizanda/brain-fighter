...
| **Game page assets** | Icon, thumbnails, description. Remember [[concepts/RobloxOpenCloudAuth]] and the `rbxthumb://` requirement for Open Cloud decals. **Drafts 2026-09-20 in [[design/game-page]]** — two description variants, icon concept, three-shot thumbnail list; upload is Creator Dashboard, not Open Cloud. Nothing uploaded yet. |
| **Analytics** | ✅ **2026-09-22.** Added 2026-07-27 (from the persistence discussion): `AnalyticsService` onboarding funnel (join → first shoot → first memorize → first cast → first boss damage → boss kill) + custom loop-health events. Pulled into the gate because the soft launch exists to observe retention — unobservable without it. See [[design/persistence-progression]]. Built as [[systems/Analytics]]: domains fire their own events on the accept branch, one listening reporter, budgeted sink; suite 2/2 and the engine acknowledged every call in a playtest. **Dashboard delivery from a published server is the one thing left to eyeball after the unlisted publish.** |
...
| 6 | ✅ **Done 2026-09-19.** `Modes/PvPDuel.luau` on two scene-authored duel pads (`ArenaSlot` tag + `ArenaId`/`Mode` attributes → sessions at boot). Exactly two — `minPlayers`/`maxPlayers` on the mode config, and a capped round fills before it starts — `allowsPvP = true`, kill limit and clock as Workspace tunables (`DuelKillLimit`, `DuelTimeLimitMin`), leaving or disconnecting mid-round is a forfeit. The portal queue became real (pool portals via `TargetMode`, intake only when a pad can start, 1 s flush; the PvE PostRound refusal now queues). Round outcome ids ride `RoundEnded` and the PostRound payload so the card reads `BOSS DEFEATED` / `OPPONENT LEFT` / `DUEL OVER` instead of `No winner`; `RoundTimerGui` ported to Builder+Config; every round starts at full health. Suite 10/10, one client-verified playtest; the 1v1 itself is a user-driven two-client check ([[design/lobby]] § Stage 6 detail). | — |
| 7 | ✅ **Done 2026-09-20.** [[systems/Lobby]] written (portals, queue, validation, counts, PortalGui, hub geometry); the [[systems/GameMode]] rewrite and the session lifecycle tests (`sessions_isolate_scores`, `transfer_moves_roster_and_attributes`, `registry_views_agree`) had already landed with refactor chunk 8. **Phase 6 closed.** | — |
...
**Decisions:** Q1–Q11 answered 2026-09-08 (recommended option on every one); recorded in [[design/refactor-plan-2026-09]] § Decisions. No chunk is blocked on input. All chunks done as of 2026-09-15; remaining loose ends live on the plan page (chunk 9's two-arena client check, tracker BRA.21 settings surface; `RoundTimerGui` was ported in Phase 6 stage 6). Phase 6 stages 5 and 6 shipped 2026-09-19.

## Plan changelog

- **2026-09-22**: **Phase 5.4 analytics built** — the onboarding funnel and six loop-health events on Roblox `AnalyticsService` ([[systems/Analytics]]). The shape decision: gameplay domains fire their own server BindableEvents on the branch that accepted the thing (`BlockConsumed`, `WordMemorized`, `MemorizeFizzled`, `SpellCast` — the first server-side signals those domains expose), and one reporter listens; funnel steps are logged once and strictly in order (a gap is reported, not filled), sends are token-bucketed under Roblox's 120/min. Verified by a new Analytics suite (2/2), the engine's own `event fired` acknowledgements, and a live pop + cast probed through the real remotes. 5.4 now has two items open: game-page upload (drafts on [[design/game-page]]) and the rollout itself.
