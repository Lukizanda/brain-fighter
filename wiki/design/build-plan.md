...
| 6 | **Required, but smaller than planned — the prediction layer already exists unnamed.** `VfxController` ← `CastAction.spellResolved` draws the caster's cast burst locally in 0.39 ms and survived Stage 4 untouched; no player spell has a `launchEffectId` to lose. So Stage 6 = name it as the sanctioned prediction path, pin the rule it must obey (draw/sound/own-HUD only; never another entity's state), and add a predicted endpoint to `spellResolved` so a future hitscan skill can draw a tracer without a redesign. | Post-launch |
