...
- [x] NPC-suite flake (`53a43d5`, committed first so the harness was trustworthy for the rest): `MoveToWithPath` no longer yields the decision tick, and a patrol route is now local to the rig. See the note below — the root cause was production code, not the fixture.
- [x] `baseWalkSpeed` attribute owned by the spawner; SkillEffects freeze applies/restores against it. `SkillConstants.BASE_WALK_SPEED_ATTRIBUTE`, stamped by `BossSpawner` and `NPCService`, rewritten by `BossController` on phase change (which no longer writes `WalkSpeed` itself while the rig is frozen). `purgeFreeze` reads it at restore time.
- [x] `AlignOrientation` on the boss HRP; the `BossStates` CFrame write removed — and the same write in `PatrollerStates` CombatState with it. Both now call `Actions.FaceTarget` / `Actions.ReleaseFacing`, which own one constraint per rig (`server/AI/Scripts/Actions.luau`).
- [x] Perception/StateMachine/Actions moved to `server/AI/Scripts/`; `BossController`, `BossStates`, `NPCController` and `PatrollerStates` requires updated. `States/` stays under `NPC/` (archetype-specific, not part of the generic layer) — documented on [[systems/NPC]].
- [x] `laserBeamEffect` → `VfxBroadcast.beam` with the client draw that already existed in `WorldVfxController`; the module moved to `src/shared/Vfx/laserBeamEffect.luau`. `Actions.MeleeAttack` + `Weapon/Melee/*` deleted. `LaserBeam.model.json` NOT checked in — see the note below.
- [x] Boss/NPC magic numbers named: new `src/shared/Boss/BossConstants.luau` (spawn drop, label offsets, attack origin height, cooldown fallback) and `NPCConstants` (`PATROL_ROUTE_MAX_RANGE`, `FACE_TURN_RATE_DEGREES`, `FACE_RESPONSIVENESS`). `BossConfig` left alone — it is a per-type registry, its numbers are data with names, not magic literals in logic.
- [x] Reduce `(bb :: any)`: 16 casts across the four files down to 2, each a documented widening in one helper (`BossStates.bossOf`, `BossController.asNPC`). `perceptionRange` became a real `Blackboard` field.
- [x] `Suites/Melee/*` — already deleted in chunk 1, no-op as expected.
...
- **Divergences (chunk 11, 2026-09-14).**
  — **The NPC-suite flake was production code, not the fixture.** The note below guessed the fixture's patrol points; those were half of it. The other half — and the reason a *different* test failed each run — is that `PathfindingService:ComputeAsync` yields, so calling it from a state's `update` fn parked the whole decision tick and the next Heartbeat started a second tick on a fresh thread. Two ticks then raced over one blackboard and a stale Patrol update issued `Humanoid:MoveTo` seconds after the machine had entered Combat, walking the rig out of the fight. `MoveToWithPath` now computes on its own thread and returns `Running` until the result lands. The commit is `fix(npc):` rather than the briefed `test(npc):` because nothing in `Tests/` changed. `ensurePatroller` and `Suites/NPC/*` were untouched.
  — **The patrol-route fix is a rule, not a fixture special case.** `NPCConstants.PATROL_ROUTE_MAX_RANGE` (200 studs) drops patrol points that are not local to where a rig stands. The four points sit within ~21 studs of `NPCSpawn_1`, and the Lobby fixture is ~900 studs away, so it now takes none and holds its ground. This also fixes the same latent bug for a real NPC spawned in a second arena.
  — **The NPC's Combat state had the same F5 CFrame write** the plan only named on the boss (`PatrollerStates` snapped `HRP.CFrame` every tick). Fixed once in `Actions.FaceTarget` as the brief asked, so both callers share one constraint implementation.
  — **`LaserBeam.model.json` was not checked in.** The plan offered it as an alternative to the broadcast route; the broadcast route landed, so the template stays a Studio instance under `Shared.Weapon.Objects`, protected by the `ignoreUnknownInstances` meta on `Weapon/` and a new `Weapon/Objects/init.meta.json`. Serialising the Part + two Beams + two Attachments faithfully is a visual-regression risk for no gain while the folder has to exist anyway for `CharacterImpact`, `EnvironmentImpact` and `ViewModels`.
  — **`Actions.cleanup` existed but was never called** — both controllers now call it on destroy. Without that, every destroyed rig pinned its own blackboard (and so its Model) through the raycast-params cache.
  — **Rojo's serve process died mid-session** (port dead, not just the plugin). Restarted it; the user reconnected the Studio plugin once. After the reconnect, empty `Weapon.Scripts` / `Weapon.Scripts.Effects` folder shells reappeared in Studio and were pruned by hand under a `ChangeHistoryService` waypoint — **the `.rbxl` needs saving** to persist that (and the earlier `Weapon.Melee` prune).
- **Verified (single client, MCP playtest, lock `chunk-11`).**
  — Harness: NPC-only **3/3 twice in a row** after the flake fix (the whole boot log fits in one console read now — the per-frame `NoPath` flood that used to truncate it is gone), then full `all` **34/34**, NPC 3/3 a third time. `RunTests` cleared after each.
  — `BaseWalkSpeed` stamped on spawn: boss **6** (phase 1), `Patroller_1` **12**. Froze the live boss through the real `SkillEffects.handlers.freeze`: WalkSpeed 6 → **0**, bumped the base to 10 mid-freeze (what a phase change now does), WalkSpeed **stayed 0**, and the freeze lifted to **10** — the new base, not the stale 6.
  — **Caveat on that check:** every shipped boss type in `BossConfig` has exactly **one** phase, and `BossPhaseManager` scans from index 2, so a real phase transition cannot occur in-game today. The base bump was applied by hand to stand in for it. F5's phase-change race is therefore latent rather than live, and `BossController`'s half of the fix (write the base always, write `WalkSpeed` only when `_frozen` is absent) is verified by construction, not by a transition.
  — Boss turning, sampled per Heartbeat over a forced ~180 degree turn on the live rig: **after** (AlignOrientation) max 4.63 deg/frame, mean 1.70, 2 direction reversals over 209 frames; **before** (the old CFrame lerp, replayed on the same rig with the constraint disabled) max 5.87 deg/frame, mean 1.48, 1 reversal over 211 frames. **The two are comparable — this metric did not reproduce the F5 jitter**, and the reversals in both runs are the two intentional target swaps. The change still stands on the ownership rule (`AutoRotate=false` + one constraint, verified live), but whoever owns F5 should know the "turn jitter" is not visible as server-side yaw oscillation; if it is real it is a client-side artifact of replicating those CFrame writes.
  — NPC tracer: with `Patroller_1` in Combat for **361/361** sampled frames, the **Server** datamodel held **0** `LaserBeam` instances while the **Client** drew **13** distinct ones in 6 s (~2/s, matching the 120 rpm rate of fire). The tracer is fully client-side.
- **Out of scope, for whoever picks it up:** `NPCService.equipWeapon` indexes `ReplicatedStorage.Shared.Weapon.Templates`, which does not exist in Studio — a hard error for any archetype that sets `weaponTemplateName` (every shipped one is `nil`, so it is latent). And `src/shared/Tests/Helpers/restoreToSafeSpawn.luau` is still on disk despite chunk 1 ticking its deletion.
