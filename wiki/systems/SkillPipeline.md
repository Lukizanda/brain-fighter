...
**The patterns are authored, not generated, and that is a bug fix.** The first version walked each crack from a random base point with random headings and lengths, and nothing stopped the walk leaving the face — a crack starting near an edge and turning outward rendered as lines hanging in the air off the side of the wall. Clamping a random walk means either steering it (which straightens cracks against the edge, exactly where they most need to look natural) or rejecting and retrying (unbounded work on a frame). A fixed atlas of ten patterns in normalised face coordinates cannot leave the slab, needs no per-wall generation, and can be looked at and tuned. Each face shuffles the atlas, reveals six, and mirrors each at random — `1 - x` is in range whenever `x` is, so variety costs nothing in safety.

Two layers keep it honest: a **startup validation pass** warns on any atlas point outside `[0, 1]`, and the segments live under a `ClipsDescendants` container, which turns "we author carefully" into something the engine enforces — segments have thickness, so one lying along an edge would otherwise put half its width past it. Measured worst overhang is 2.3 px (0.06 studs), under half a segment's thickness, and clipped away.
