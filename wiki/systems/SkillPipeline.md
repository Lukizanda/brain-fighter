...
The server broadcasts launch parameters as the `projectile` kind of `VfxBroadcast` → `WorldVfxEvent` (the separate `ProjectileVfxEvent` remote and `ProjectileVfxController` were folded into the world lane in refactor chunk 7); each client's `WorldVfxController` replays them locally. This is safe to predict because a shot is a **straight line at constant velocity** — the client reproduces the exact path from the launch parameters alone, with no correction traffic. It is not guessing at the outcome, it is evaluating the same geometry against state it already has (`_shield` replicates, and the bubble is drawn from it). Divergence is bounded by construction: if client and server disagree about a block, the client loses a cosmetic and the server still decides who took damage.
...
It's driven by `src/client/Vfx/ShieldVfxController.client.luau` watching `_shield` (through `CharacterAttributeWatcher`), **not** by `SkillEffects.handlers.shield` calling into it. That controller also owns the one gameplay number the bubble needs — `SkillBuffs.SHELL_RADIUS_STUDS * 2`, passed to `ShieldVfx.update` — because `ShieldVfx` itself may not require the Skills layer. That's forced by the ownership split above: Skills grants the pool and Health drains it, so a start-on-cast hook would have had no matching stop-on-absorb hook. The attribute is the one place both systems meet. Freeze was converted to the same shape on 2026-08-04 — see [[systems/VisualEffects]] — so this is now the pattern for every status visual rather than a shield-specific workaround.
...
| **Status visuals** | A module under `src/shared/Vfx/StatusVisuals/`, always driven by a client watcher on the status's own attribute — `ShieldVfx` ← `client/Vfx/ShieldVfxController` (`_shield`), `FreezeVfx` ← `client/Vfx/FreezeVfxController` (`_frozen`), `InfernoVfx` ← `client/Vfx/InfernoVfxController` (`_burning`), `ChargeOrbVfx` ← `client/Vfx/ChargeOrbController` (`_chargeColor`/`_chargeTier`). Since 2026-09-11 all four controllers are handler pairs over one `Vfx/CharacterAttributeWatcher` (see [[systems/VisualEffects]] § "Who each status watches" for the per-controller flags). Never called from `SkillEffects.handlers.<kind>` directly; all four modules refuse on the server. | Persistent geometry or emitters attached to the rig, living for the duration of the active status; cleanup is reciprocal (`start` + `stop`) | The effect is a persistent body decoration (ice shards, flames, poison cloud) that must follow limbs and survive a multi-second status. Burst particles can't carry these semantics. |
...
Cross-client replication: burst VFX replicate explicitly, and only one way — the authoritative run raises them via `SkillVisuals` → `VfxBroadcast` → `WorldVfxEvent` (the client-originated `BroadcastSpellVfx` relay was deleted 2026-09-08; the caster's client predicts the cast cue alone). Delivery visuals are local to the firing client. Screen-space effects don't replicate at all — each client decides its own. **Status visuals ride attribute replication** — the authoritative value is written on a replicated character Model, so every client's watcher sees it without a RemoteEvent, and a late joiner sees an already-frozen, already-shielded or already-burning rig because the value is simply there when they arrive. As of 2026-08-04 all status visuals work this way; freeze used to be local to whichever VM ran the handler, which double-drew it for the caster.

**Level-triggered status, edge-triggered punch.** A watcher that also fires a one-shot (as `InfernoVfxController` does, driving `ScreenImpact` on ignition) must distinguish the two, because `refresh` cannot: the attribute signal fires on *any* write including a server re-write of the same value, and the initial read on bind looks identical to a fresh application. `InfernoVfxController` keeps a `lit` map per watched rig and punches only when the watcher's `initial` flag is false (the flag exists on `onRefresh` for exactly this). Without both, a player walking into the arena mid-burn gets hit by a detonation that happened before they arrived, and a redundant write re-punches every watching screen.
