...
> - `src/shared/Vfx/VfxBroadcast.luau` + `src/client/Vfx/WorldVfxController.client.luau` — **the server's only legal way to make a player see or hear something** (2026-08-04), and since chunk 7 the *only* cross-client VFX lane. Kinds: `point`, `attached`, `beam`, `collect`, `shockwave`, `projectile`, `telegraph` (the last added by chunk 10, 2026-09-11, which retired `BossWindupEvent`). See § "Nothing player-facing runs on the server" below.
> - `src/shared/Vfx/Remotes/WorldVfxEvent.model.json` — the one remote. `BroadcastSpellVfx`, `SpellVfxEvent`, `ProjectileVfxEvent` and `server/Vfx/VfxBroadcastService` were deleted 2026-09-08; `Boss/BossEvents/BossWindupEvent` and `client/BossWindupClient` followed on 2026-09-11 (refactor chunk 10) — the windup telegraph is now a `telegraph` payload on this lane.
> - `src/shared/Vfx/CharacterAttributeWatcher.luau` — **the binding half of every attribute-driven status visual** (chunk 10). One factory the four status controllers share: rig table keyed by Model, cached Humanoid, both exits (`Humanoid.Died` + `AncestryChanged`), the `initial` refresh a late joiner needs, and the tagged-rig retry for a rig tagged before its Humanoid is parented. `enable()` / `disable()` / `destroy()`; `disable()` detaches every rig so the handlers take their geometry off. It holds the **one** `Skills` require left in `src/shared/Vfx` (`SkillConstants`, for `DAMAGEABLE_TAGS`) — see § "The Vfx layer does not depend on the Skills layer".
> - `src/shared/Vfx/MotePath.luau` — the curved flight of a mote toward a *moving* destination (quadratic Bézier, bowed control point in a random roll, per-mote flight jitter), shared by `ChargeOrbVfx` and `collectStream`, which each had their own drifting copy. The numbers (ring radius, size, curve amount, flight length) stay with the callers: the two effects are not supposed to look the same.
...
> - `src/shared/Vfx/ScreenImpact.luau` — the screen-space lane (2026-08-08): warm `ColorCorrectionEffect` + `BlurEffect` in `Lighting`, plus a camera shake via `Humanoid.CameraOffset`. Deliberately **not** `Camera.CFrame` — the default Roblox camera scripts own that and rewrite it every frame. **Skipped outright when the player's Reduced-effects setting is on** (2026-09-20, [[systems/Settings]]); `spawnEffect` honours the same flag by dropping lights and scaling every `emitCount` by `PERF.reducedVfxEmitScale` (0.5).
...
> - `src/shared/Vfx/StatusVisuals/ChargeOrbVfx.luau` + `src/client/Vfx/ChargeOrbController.client.luau` — the hold-to-charge orb (2026-08-12, [[systems/ChargeCast]]). Copies ShieldVfx's `start`/`update`/`stop` shape and its `_chargeColor` / `_chargeTier` attribute wire, and is the first persistent visual that must be *grown* while it runs. **It cannot be a `spawnEffect` entry**, and the reason is worth pinning: `spawnEffect` returns nothing, hands every instance it makes to `Debris` with a duration fixed at author time, and its only sustained branch is a `task.delay` that flips `Enabled = false`. A charge has no author-time duration and needs a handle. The mote halo's *look* still lives in `VfxConfig` as `charge_motes_<colour>` (rate-based `EmitterSpec`, resolved by `VfxConfig.resolveChargeMotesId`); only its lifetime is owned by the module. The converging mote Parts fly `MotePath`'s quadratic-Bézier curve — shared with `collectStream` since chunk 10 rather than copied from it — with the orb as the destination and a ring around the caster as the source. Since chunk 10 the module also takes its two gameplay facts as **inputs** (`ChargeInputs`: the roster's top tier, and this colour's spell name per tier) instead of requiring `SpellRegistry`; `SpellMenuGui` builds them for the caster's own orb and `ChargeOrbController` for everyone else's. A third layer arrived the same day: a `BillboardGui` **name label** over the orb, popping in on each tier crossing and dissipating on release — parented to the head rather than the orb, because its dissipate has to outlive the orb's release flash. It is the project's only world-space text and its `AlwaysOnTop` is deliberately off (see [[systems/ChargeCast]] § The name label).
>
...
>
> **Corrected contract facts:** the `impactEffectIds` / `MAX_TIER` / `MAX_IMPACT_EFFECT_IDS` relay contract described below went with `VfxBroadcastService` in refactor chunk 7 — `VfxBroadcast` messages carry one `effectId` per `kind` (`point` / `attached` / `beam` / `collect` / `shockwave` / `projectile` / `telegraph`), and the cast tier is validated by [[systems/SpellCastService]] through `SpellRegistry.getSpell` (`MAX_ENABLED_TIER = 3` since T4 was parked 2026-09-23). Lifetime cleanup is **`Debris:AddItem` only** today — the §"PERF guardrails" cap/evict/throttle table and the `"VfxInstance"` Heartbeat sweep are **not implemented** (`PERF` data exists but `spawnEffect` reads none of it). The LetterBlock collect-pop is still unbuilt. Green cast entries **were** unbuilt until 2026-08-03 — `cast_green_t1..t3` now exist, and `spawnEffect` plays `EffectSpec.sound` (it previously consumed only `emitters`, so every spell was silent). **`.light` is implemented as of 2026-08-08** — a `PointLight` at full brightness on the landing frame, tweened to 0 over `durationSec` and honouring `PERF.allowLights`; `impact_damage_t3` is its first and so far only consumer. `.beam` is still ignored.
...
### Who each status watches (refactor chunk 10, 2026-09-11)

The four controllers are now handler pairs over one `CharacterAttributeWatcher`, so "which rigs" is a flag rather than forty duplicated lines each. The flags are not uniform, and the asymmetry is the design:

| Controller | Attribute(s) | Tagged rigs? | Local player? | Why |
|---|---|---|---|---|
| `FreezeVfxController` | `_frozen` | **yes** | yes | Freeze is aimed at the boss more often than at anything else. |
| `InfernoVfxController` | `_burning` | **yes** | yes | Same; plus the edge-triggered screen punch on ignition. |
| `ShieldVfxController` | `_shield` | no | yes | The absorb pool comes from selfTarget spells, so only a player can hold one — and both shell tests (`SkillDelivery` server-side, `WorldVfxController`'s `shellContactFor` client-side) scan players alone. A bubble on a boss would be a bubble nothing deflects against. |
| `ChargeOrbController` | `_chargeColor`, `_chargeTier` | no | **no** | The caster's own orb is drawn by `SpellMenuGui` on the press frame; a second writer would fight it with round-trip-late values. Tagged rigs were dropped in chunk 10: nothing writes the charge attributes on a rig, and a boss winding up now has its own lane (`SkillVisuals.spawnTelegraph`). |

If boss shields ever land, three places change together: `includeTaggedRigs` here, the client shell scan, and `SkillDelivery`'s.
...
## The Vfx layer does not depend on the Skills layer

`grep "require(.*Skills" src/shared/Vfx` returns exactly one line: `CharacterAttributeWatcher` requiring `SkillConstants` for the damageable-tag list. That is the gate chunk 10 put in place, and it is worth stating as a rule rather than a grep: **Skills may require Vfx; Vfx must not require Skills.** Presentation is the lower layer and cannot be allowed to ask gameplay questions.

Three modules used to break it, and each was fixed by taking the gameplay fact as a parameter rather than reading it:

- `ShieldVfx` drew its bubble at `SkillBuffs.SHELL_RADIUS_STUDS * 2`. `update`/`start` now take `diameterStuds`; `ShieldVfxController` supplies it, so the drawn bubble and the sphere `SkillDelivery` stops projectiles against still cannot drift.
- `ChargeOrbVfx` queried `SpellRegistry` for the top tier and the spell names. Both arrive as `ChargeInputs` at `start`.
- `CosmeticProjectile` required `SkillBuffs`, `Hittables` and `SkillVisuals`. Its `LaunchParams` gained three injected functions — `drawAt` (play an effect at a point), `shellContactFor` (where a step meets a shield shell) and `rigWithin` (the server's proximity rule) — all wired by `WorldVfxController`, which is a client script and may require whatever it likes. The ray-sphere maths stayed in `SkillBuffs.shellEntry`, shared with the authoritative block: duplicating it would be a desync waiting to happen.
...
- **Fired by**: `VfxBroadcast.*` only, and `VfxBroadcast` refuses (loudly) on a client. Callers are `SkillVisuals` (cast cue, impact cues, shockwave, barrier overlay, telegraph), `SkillDelivery` (`projectile`), `BlockShootService` (`collect`).
...
| `telegraph` | `VfxBroadcast.telegraph` | `SkillVisuals.spawnTelegraph` — a rig's windup warning: `aoe_indicator` (a ground disc covering the danger zone, pulsing faster as the window closes) or `charge_glow` (a ball over the rig that swells). Carries the rig as an Instance, which is safe because a boss is long-lived; no `drawnLocallyBy`, because nothing predicts a warning about someone else's attack. |
