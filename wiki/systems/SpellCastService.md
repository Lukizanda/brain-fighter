...
**Reading the shadow log — superseded 2026-09-10.** `DevDebug`'s `[` hotkey used to conjure letters into the buffer without telling the server, so every dev memorize logged a coverage failure and the standing advice was to discount any would-reject line following a `[` press. That advice is retired: `[` now credits the ledger's held set through `DevCreditLetter`, so the `]` that follows is checked against letters the server agrees the player holds. **A rejection after a `[` press is a real finding again.**

**The dev cheats need a server half (2026-09-10).** `DevDebug`'s `1`-`4` keys fill the client's `EnergyReservoirs` directly, which was the whole cheat while `ENFORCE` was false. Enforcement made those casts unaffordable — the ledger credits only spelled-for energy, so a cheated cast dies at `checkCast` with `red cast costs 5, ledger has 0` after reporting `ok` on the client. Reported as "spells are broken", which is how a stale cheat presents. The keys now also fire `Shared.Economy.Remotes.DevGrantEnergy`, and `EconomyService` writes the matching ceiling through `EnergyLedger.devSetCeiling` (set, not add, so pressing `2` after `4` lowers both halves together). The wire carries the **tier**, not an amount, and the handler is gated on `RunService:IsStudio()` — a remote that hands out energy is not something to leave one config flag away from a live server.

The `[` key had the same split for the same reason, and gets the better repair: `DevCreditLetter` credits the conjured letter to the held set via `EnergyLedger.creditBlock`, the same call the accepting branch of `BlockShootService` makes. The `]` that follows then runs the **real** `ReportMemorize` path — real coverage check, real `EnergyEconomy` split — so a dev word earns exactly what a spelled word earns. Only the letter's origin is faked, not its worth, which is why this is the shape to prefer when a dev path has to cross the client/server line. Verified 2026-09-10: seven `[` presses and one `]` logged `ZandaLuki memorized CATALOG`, and blue and red T1 casts funded by that word alone both reached `(server apply)`. See [[concepts/DevDebugHotkeys]].
