...
- `src/shared/BlockShoot/init.luau` — shared helpers: `findLetterBlock` (ancestor traversal), `readBlock` (attribute reader), `resolveHandle` (the staff part that carries the muzzle and the Sounds), `muzzlePosition` (where a shot leaves the staff), `MAX_RAYCAST_DISTANCE` constant.
...
A validated consume ends with `broadcastShot`, which resolves the firing player's muzzle via `BlockShoot.muzzlePosition(tool)` and calls `VfxBroadcast.beam(origin, blockPos, opts)` — carrying both the laser and the name of the fire sound to play.
...
- **The sound is named, not shipped.** The payload carries the shooter's Handle plus `LetterBlasterConfig.FIRE_SOUND_NAME`, and each client plays its own already-replicated copy of that Sound — keeping the authored volume and rolloff, and staying positional because the Handle is a BasePart. Shipping a raw `SoundId` instead would have meant re-tuning distance falloff at the receiving end and letting it drift from the instance a designer can actually see.
