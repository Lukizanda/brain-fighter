...
## [2026-09-10] ingest | The `[` / `]` dev word earns real ledger energy

Follow-up to the mana-cheat fix, same split-brain one key over. `DevDebug`'s `[` conjures a letter into the buffer without popping a block, so the ledger never held it and `]` failed coverage. Reproduced through real key input: seven `[` presses then `]` gave `[MemorizeAction] memorize ok — CATALOG` on the client and `[EconomyService] rejected memorize from ZandaLuki — claimed 2 x A/blue, holds 0` on the server — under ENFORCE the client banks energy the server will not let any cast spend. Fixed with a new `Shared/Economy/Remotes/DevCreditLetter`: `[` reports the conjured letter and `EconomyService` credits it with `EnergyLedger.creditBlock`, the same call `BlockShootService` makes on an accepted consume. Deliberately a credit and not a grant — the `]` then runs the real `ReportMemorize` path, real coverage check and real `EnergyEconomy` split, so only the letter's ORIGIN is faked and a dev word earns exactly what a spelled word earns. Verified: `[EconomyService] ZandaLuki memorized CATALOG`, then blue and red T1 casts funded by that word alone both reached `(server apply)` and damaged the dummy. Harness 34/34. The standing "discount any would-reject line after a `[` press" advice is retired on both pages — a rejection there is a real finding again. Pages touched: [[concepts/DevDebugHotkeys]], [[systems/SpellCastService]] § Validated memorize.


## [2026-09-11] ingest | Refactor chunk 10 — presentation dedupe and layering

The four attribute-driven status visuals (freeze, burn, shield, charge orb) now share one binder: new `src/shared/Vfx/CharacterAttributeWatcher.luau` owns the rig table, the cached Humanoid, both exits (`Died` + `AncestryChanged`), the `initial` refresh a late joiner needs and the tagged-rig retry, with `enable()`/`disable()`/`destroy()`. Each controller is reduced to its handler pair plus two flags, and the flags are deliberately not uniform: Freeze and Inferno watch tagged rigs (freeze is aimed at the boss more than anything else), Shield and Charge watch players only — Shield because the absorb pool is selfTarget-only and both shell tests scan players, so a bubble on a boss would deflect nothing; Charge because nothing writes the charge attributes on a rig and the boss windup now has its own lane. The controllers also tear down on `script.Destroying`, since Rojo replaces them on every sync and each replacement used to strand connections and welded shards.

**`grep "require(.*Skills" src/shared/Vfx` is down to one line** — `SkillConstants`, for the damageable-tag list — which is the point of the chunk: Skills may require Vfx, Vfx may not require Skills. The three offenders now take the gameplay fact as a parameter. `ShieldVfx` is handed `diameterStuds` (still `SkillBuffs.SHELL_RADIUS_STUDS * 2`, now resolved by the controller). `ChargeOrbVfx` is handed `ChargeInputs` — the roster's top tier and this colour's tier names — instead of querying `SpellRegistry`, which also retires the `getSpell` pcall guard: a colour that tops out at T3 simply has no fourth name. `CosmeticProjectile` takes `drawAt`, `shellContactFor` and `rigWithin`; `shieldedRoots` and the two closures *moved* into `WorldVfxController`, a client script that may require what it likes, and `SkillBuffs.shellEntry` stayed put so the drawn block and the authoritative one cannot disagree about the same sphere.

New `src/shared/Vfx/MotePath.luau` is the one copy of the curved mote flight (quadratic Bézier, control point bowed in a random roll around the travel axis, per-mote flight shortened by a jitter fraction), shared by `ChargeOrbVfx` and `collectStream` which had drifted copies. The numbers stay with the callers — a block's take is a quick shower, a charge is a slow inward pull — and the jitter bands are named constants on both sides.

The boss windup telegraph lost its private lane: `BossWindupEvent` and `client/BossWindupClient.client.luau` are deleted, `BossStates` calls `SkillVisuals.spawnTelegraph(style, rig, durationSec, radiusStuds)`, and that routes server→`VfxBroadcast.telegraph` / client→draw like every other primitive in that module. Both renderers moved with it and their inline tuning became `TELEGRAPH_*` constants. `WorldVfxEvent` is again the one server→client VFX lane.

Verified, single client (one playtest, lock `chunk-10`): harness `all` **34/34**, `RunTests` cleared. With the statuses written from the server, the **Client** datamodel showed 15 `IceShard` welded to the rig, 16 Inferno instances in it and 1 `SpellShieldBubble`, all going to 0 with no strays when the attributes cleared. After taking the PvE pad the client rendered both telegraph styles off the new lane — `TelegraphChargeGlow` growing 3 → 5.4 studs in 0.35 s, `TelegraphAoeIndicator` at `0.25 × 44 × 44` (GroundSlam's 22-stud radius) — and a boss volley drew 11 `CosmeticProjectile` Parts, so the injected seams fly. The charge orb is the one visual a single client cannot check (the controller skips the local player by design), so it leads the two-client snippet. Rojo removed both deleted Studio instances itself; the `.rbxl` needs saving.

Pages touched: [[systems/VisualEffects]] (new § "Who each status watches", new § "The Vfx layer does not depend on the Skills layer", telegraph row on the kind table), [[systems/SkillPipeline]] (status-visual rows), [[systems/ChargeCast]] (players-only watcher, `ChargeInputs` for the name label), [[design/client-server-boundary]] (the retired `BossWindupEvent` lane), [[design/refactor-plan-2026-09]] § Chunk 10.
...
## [2026-09-20] ingest | Arena instancing — future-work plan, decision = stay single-server

New `design/arena-instancing.md` written the day after stage 6, prompted by "should a new duel pad spawn when the hub is busy?" Compares the three Roblox patterns (reserved match servers, fixed in-server arenas, in-server dynamic instancing); records the user's decision to stay on fixed scene-authored pads and author more before writing code. Costs both switch paths against the actual `src/` seams — `DeathZoneService`/`SpawnManager`/`livePortals`/`Hittables` already runtime-safe, only `BlockSpawnerService` and `createSceneSlots` scan once at boot — with Mermaid diagrams and a teardown-race warning. Linked from `index.md` and `design/lobby.md` (not-in-stage-6 list).

## [2026-09-20] ingest | Phase 6 stage 7 — systems/Lobby written, Phase 6 closed; game-page drafts

New `systems/Lobby.md`: ownership table, the two portal shapes (`TargetArenaId` / `TargetMode`), the queue algorithm as a flowchart (`pruneQueue` → `candidateSessions` fullest-first → `intake` → `transferPlayer`), the validation table, published counts and sign copy, `PortalGui` behaviour, player state, the hub geometry, constants, files, verification history, and the test gap (queue logic is Script-local; extract `LobbyQueue` if it grows rules). Stage 7 row ticked on `design/build-plan` and `design/lobby`; `systems/GameMode` description no longer says "Lobby and NoOp". New `design/game-page.md` drafts the Phase 5.4 store assets (descriptions, icon concept, thumbnail shot list, upload path) — nothing uploaded.

## [2026-09-20] lint | BRA.20 wiki lint re-run after Phase 7 + Phase 6

Mechanical pass over 60 pages (script: wikilinks/orphans, `src/` citations, `updated:` vs git). 0 orphans; the 6 broken wikilinks are `WIKI.md` template placeholders and historical `log.md` entries. 59 dead `src/` citations, 56 on REMOVED / self-flagged / planning / log pages by design; 3 fixed on live pages — `systems/AudioSFX` (weapon `playSoundFromSource` marked historical), `concepts/ModelJsonInstances` (melee example → `PortalRequest.model.json`), `design/tap-to-pop` (`Actions.luau` moved to `server/AI`). 8 `updated:` bumps where chunk 7/10/11 docs edits skipped the frontmatter, plus `index`. `git log --name-only` since `c026a03`: every `src/` commit without a wiki file was covered by a docs commit in the same batch (chunk 12's `79298f8`, chunk 11's NPC fix in `systems/NPC`). The 12 design-intent drifts in `design/system-audit-2026-09` cross-checked and annotated: all closed.

## [2026-09-20] ingest | Settings surface — SFX volume, Reduced effects, letter palette (BRA.21)

New `systems/Settings.md`. `Settings` is a pure module (keys, defaults, normalize on read and write, attribute IO on any holder — LocalPlayer in the game, a Folder in the Unit suite). `SettingsController` owns a `SoundService.Sfx` SoundGroup and routes every Sound into it (scan + DescendantAdded on SoundService and workspace, one writer of `SoundGroup`), and applies the palette through `Colors.setPalette` plus a repaint of every tagged block. `Colors` now owns `PALETTES` (default = the LetterBlocks tints; high_contrast = Okabe–Ito) and `tint()`; block faces, hover fill, buffer tiles and spell-menu discs (incl. ready halo + motes on `repaint`) draw through it — the Phase 5.3 "colour type dedup ×4" closed as a side effect. `spawnEffect` drops lights and halves bursts, `ScreenImpact` skips, when Reduced effects is on. `SettingsGui` = gear in TopRight + panel at Modal, both `LobbyOnly` (first users). Pages touched: HUD, HudGate, LetterBlock, VisualEffects, Tests, index, build-plan (5.3 row + changelog).

## [2026-09-22] ingest | Phase 5.4 analytics — onboarding funnel + loop-health events

New `systems/Analytics.md`. Roblox `AnalyticsService` via one listening Script (`AnalyticsReporter`) over a pure `OnboardingFunnel` (once, strictly in order, gap reported not filled, refused send retries) and an injectable, token-bucketed, pcall-wrapped `AnalyticsSink`. Domains gained server BindableEvents on their accept branches — `BlockShoot/Events/BlockConsumed`, `Economy/Events/{WordMemorized,MemorizeFizzled}`, `SpellCast/Events/SpellCast` — and `BossSpawner` exports `BOSS_TAG`. `Analytics_FunnelStep` / `Analytics_Events` published on the Player. New `Suites/Analytics` (2 tests). Verified in one playtest: suite 2/2, the engine's `event fired` acknowledgement on all ten sends, and a real `ConsumeBlock` + `SpellCastServer` observed firing the new events. Pages touched: index, build-plan (5.4 row ✅ + changelog), Tests, BlockShoot, SpellCastService, Boss.

## [2026-09-22] ingest | Portal signs cleared the lobby arches (per-portal `SignHeight`)

Polish: the PvE/PvP signs hung inside their arch. `Lobby.SIGN_HEIGHT_STUDS = 16` is measured from the pad centre and suits the return pads (waist-high walls), but the two lobby arches run pillars to Y=226 and a lintel to Y=230 over a pad at 203.5 — the sign landed at 219.5, in the doorway. Rather than lift every portal, `Lobby.Attributes.SignHeight` is a new optional per-portal override that `PortalGui.signHeightOf` reads with the constant as fallback; both lobby pads carry `SignHeight = 38` (scene attribute, alongside `ModeLabel`/`TargetArenaId`). 38 rather than the ~28 the arch's extents suggest: a `BillboardGui` is a camera-facing plane at the pad's own depth, so the lintel's near top edge occludes it from well above the arch — tuned by client screenshot at eye height from 30, 25 and 13 studs out, not from geometry. Return-pad signs unchanged at 16. Pages touched: `systems/Lobby`.

## [2026-09-22] ingest | Training dummy health readout — bar, damage numbers, 1s respawn

The hub dummy took spell hits in silence, so there was nowhere to find out what an attack spell is worth. New `TrainingDummy` tag (`HealthConstants.TRAINING_DUMMY_TAG`), separate from `Damageable` so tagging a rig "can be hurt" does not put a bar over every NPC. `TrainingDummyBuilder` + `TrainingDummyConfig` draw a world-space bar with a `current / max` line and pop a rising `-N` per hit; `TrainingDummyGui` finds the tagged rigs and binds `Humanoid.HealthChanged`. No remote: Health already replicates, so the delta between two replicated values is the damage `applyDamage` computed — hits inside one replication batch read as one combined number, which for a practice target is the honest total. The dummy has no `maxHealth` attribute, so it sits on `DEFAULT_MAX_HEALTH` = 100, the same as a player, and its numbers transfer directly (damage is `fractionOfMaxHP`). Death needed no new code — `DeathHandler` already fires an `Explosion` plus 8 flung fragments, both of which replicate; `respawnTime` authored at 1s so the fresh clone stands up inside the 3s debris. Verified in one playtest from the client: bar at 40/100 in the warning colour with `-20` in flight, client-side `DescendantAdded` counting Explosion x1 + fragments x8, and three death/respawn cycles each re-attaching a readout. Pages touched: `systems/Health` (new § Training dummies), `systems/HUD` (new § World-space builders).

## [2026-09-22] ingest | Dummy readout generalised to a combatant nameplate; NametagService retired

Follow-up to the training-dummy readout the same day. The dummy is meant to teach the display a real opponent shows, but opponents showed only a name (`NametagService`, server, no health), so the dummy was teaching a screen that did not exist. `TrainingDummy{Builder,Config,Gui}` → `Nameplate{Builder,Config,Gui}`, now attached to every *other* player's character as well as every `TrainingDummy` rig, through one builder — if the two diverge the tutorial is lying. `NametagService` deleted: one head cannot have two BillboardGui owners, and the health half has to be client-side since it is computed from replicated `Humanoid.Health`. Names are client-drawn now; `PlayerToHideFrom` became "skip the local player". Dummy carries a `NameplateLabel` attribute ("Training Dummy") so the plate does not read an instance name.

**Distance scaling: the fix was deleting code, not adding it.** A `BillboardGui` sized in `Offset` already renders at a fixed pixel size at any distance — measured by pinning the plate at 220 px and capturing at 20 and 70 studs, where the bar spans an identical width while the rig shrinks away. A draft that rescaled the plate per `RenderStepped` to cancel a falloff that does not exist was removed; it would have made the plate *grow* as you backed away, i.e. introduced the exact scaling it was meant to prevent. `AbsoluteSize` is a trap here — inside a BillboardGui it reports the billboard's own canvas (it equalled `Size.Offset` at every distance) and says nothing about screen pixels. Verified in one playtest: plate on the dummy reading `Training Dummy / 100 / 100`, plate on a live player rig via a temporary tag with exactly 1 BillboardGui on the head (no nametag duplicate). Pages touched: `systems/Health` (§ Training dummies → § Nameplates), `systems/HUD`, `systems/GameMode`.

## [2026-09-22] fix | Nameplate sized in studs — it was a banner over a distant dummy

Reported from the lobby spawn: the plate covered the whole dummy and looked huge. Root cause is the sizing unit, and the correction reverses the note in the entry above. A `BillboardGui` sized in `Offset` holds a fixed *pixel* size at any distance — true, measured, and exactly wrong for a nameplate: at 137 studs (the nearest `LobbySpawn` to the dummy) a 220 px plate is a banner over a rig the size of a thumbnail, covering the dummy and the portal signs behind it. `PLATE_SIZE` and `DAMAGE_SIZE` are now in studs (`Scale`), so the engine shrinks them with the rig; `CORNER_RADIUS`, `BORDER_THICKNESS` and `TEXT_INSET` follow, since pixel trim is meaningless against a canvas that is ~17 x 2.4 units at range. `NAME_ROW_HEIGHT`/`BAR_ROW_HEIGHT` renamed to `*_WEIGHT` — they were only ever a ratio. Cost: at ~35 studs the fill fraction reads but the numbers do not, which is the right gradient (numbers at the dummy you walk up to, fraction across a duel pad). Verified in one playtest at 137, 35, 18 and 14 studs, including a `-20` popup at 18. `AbsoluteSize` note for next time: it mirrors `Size.Offset` for an Offset-sized billboard (useless) but tracks projected size for a Scale-sized one (168 px at 14 studs, 17 px at 137). Pages touched: `systems/Health`, `systems/HUD`.

## [2026-09-22] fix | Two health bars over the dummy — the engine draws one too

Reported: the dummy showed two health bars and two names. Not a duplicate instance — there was only ever one `BillboardGui` on the Head. The second is the **engine's**: a `Humanoid` draws its own name and, when damaged, its own health bar, so `Humanoid.DisplayName` ("Practice Dummy") sat under the plate's label ("Training Dummy") with a second bar above it. It hid through development because `HealthDisplayType` defaults to `DisplayWhenDamaged` and every screenshot was of an undamaged 100/100 dummy. `NameplateGui.attach` now sets `DisplayDistanceType = None` per rig — that covers name and bar together, where `HealthDisplayType` alone would leave the engine's name. Written client-side, where the plate lives and where the property matters.

Same pass, user's call: the bar's `current / max` text is gone. Unreadable at any range worth reading a bar at, and the floating damage number already says what a hit was worth. The bar keeps the job it does well from a distance — a fraction — and got slimmer for it (`PLATE_SIZE` 1.3 → 1.0 studs tall, row weights 22/34 → 62/38 so the name is the taller row). `TEXT_FONT`/`TEXT_COLOR`/`TEXT_STROKE_TRANSPARENCY`/`TEXT_INSET`/`TEXT_FORMAT` deleted. Verified live in the user's own running playtest without restarting it — engine plate silenced, one slim bar at ~60% fill. Pages touched: `systems/Health`.

## [2026-09-22] ingest | Nameplate layout expressed as three studs, not a ratio

The plate's vertical layout was a fixed height split by two weights, which coupled every knob to every other: the name's font size could not move without changing the bar, and there was no gap between them to change at all. `NameplateConfig` now states the layout as three independent measurements in studs — `NAME_HEIGHT_STUDS` (which *is* the font size, since the label is `TextScaled` and its glyphs fill the row), `ROW_GAP_STUDS`, `BAR_HEIGHT_STUDS` — and the builder sums them for the plate height. `PLATE_SIZE`/`NAME_ROW_WEIGHT`/`BAR_ROW_WEIGHT` replaced by those plus `PLATE_WIDTH_STUDS`. Caveat recorded in the config: a long name runs out of width before height and shrinks to fit, so `NAME_HEIGHT_STUDS` is the largest the name will draw, not a guarantee. Verified in one playtest at 16 studs: 0.66 / 0.10 / 0.54 studs, plate 1.30 tall, gap visible and the bar unchanged from the previous commit.

## [2026-09-22] ingest | Nameplate session closed — carry-forward recorded

Session wrap for the nine commits `422cc4a..1e55eaa` (portal signs, dummy readout, shared nameplate, studs sizing, engine plate silenced, bar/spacing knobs). Two things moved out of chat and into `systems/Health` so a fresh session finds them: **§ Verification status** — the plate is proven on the dummy and on a player rig via a temporarily tagged local character, but `watchPlayer` has never run against a real second client, which is the one branch a single-client playtest cannot reach; and **§ Scene state, not repo state** — the `TrainingDummy` tag, `NameplateLabel`, `respawnTime = 1` and both pads' `SignHeight = 38` live in the `.rbxl` like the portals' existing `ModeLabel`, so a hub rebuild drops them and the code falls back to defaults silently.

## [2026-09-23] ingest | T4 parked — the roster stops at three tiers

Design call, user's: disable tier 4, because it is not well defined. Only red ever defined one — `Volley`, authored as a projectile proof-of-concept (3 × flat 12) that the Inferno one rung below it out-damages — so the top of the ladder was strictly worse than the middle and had no design to balance against. Parked rather than rebalanced: what a fourth tier is *for* (finisher? school ultimate? cross-colour cast?) is the open question, and picking numbers before answering it is how Volley happened the first time.

**One constant does it.** `SpellRegistry.MAX_ENABLED_TIER = 3`; a loop after the `SPELLS` table trims every school to it, and `NUM_TIERS` is now that constant rather than `#TIER_COSTS`. The `Volley` spec literal and the 40-mana rung both stay in the file, so un-parking is a one-line change. Nothing else in `src/` needed touching — the cast panel's rings, the charge orb's scale, `CastAction.resolveSpecAtCharge` and `ChargeStateService`'s clamp all already counted tiers from `tierCount(color)`, which existed precisely because red was the odd school out. `getSpell("red", 4)` now **errors** instead of returning nil, so `SpellCastValidation`'s existing pcall turns a forged T4 cast into `unknown spell red/4` — checked live on the Server VM.

Verified in one playtest: `RunTests="all"` → **43/43 passed, 0 failed**, attribute cleared after. Both VMs log `Loaded — 3 colors × up to 3 tiers`. Tests updated where they pinned the old ceiling: red@40 now lists 3 spells, a new case asserts `tierCount == 3` for all three schools, `getSpell("red", 4)` is now an expected-throw case, and `CastAction`'s long-hold scenarios derive their duration from `NUM_TIERS` instead of naming tier 4.

**Two consequences flagged, not fixed** (both recorded on the pages, both tuning calls for the user): the 60 reservoir cap was sized as "one T4 cast plus change" and now buys three T3 casts with surplus left over; and the longest hold in the game fell from 7 s to 3 s, which shrinks the charge orb's PvP tell — the outermost cast panel ring went with it, leaving the outer two-thirds of every disc unmarked. Pages touched: `design/gameplay-loop` (the park's rationale + the roster and threshold tables), `systems/SpellRegistry` (new § "T4 is parked"), `systems/ChargeCast` (new § "What parking T4 took off the panel"; the pre-park verification runs left as the record they are), `design/build-plan`, `concepts/DevDebugHotkeys`, `index`.

## [2026-09-23] audit | Post-refactor sanity check — wiki, graph, Studio tree, trackers

- **Latent fresh-clone break found and fixed.** `Shared/Health/Remotes/{DamageConfirm,DamageFeedback}.meta.json` and `Server/Health/Events/{PlayerDamaged,PlayerEliminated}.meta.json` were *lone* meta files (no sibling script/folder) since the initial commit. Rojo 7.7 creates nothing from that pattern — its `/api/read` tree showed both folders empty — and the four events the damage path depends on lived only in the `.rbxl`, in a second Studio-only `Remotes` / `Events` folder kept alive by the parent's `ignoreUnknownInstances`. Code worked only because `FindFirstChild` returned the Studio copy first. Renamed the four files to `.model.json`, collapsed the duplicate folders in Studio (one `Remotes`, one `Events`, two children each), and re-ran `RunTests="all"`: **43/43** — `multiplayer_invariants` covers exactly these events. [[concepts/ModelJsonInstances]] § The trap and [[concepts/RojoJsonValidator]] corrected (the validator's "Rojo creates these in many cases" claim was wrong); `tools/validate_rojo_json.py` docstring updated, rule not yet hard-blocked.
- **Knowledge graph was stale.** `graphify update .` (even `--force`) kept 166 deleted files — the whole Weapon/Firearm/Loadout/LetterBlaster/Spelling Staff/SettingsMenu tree — because its `cache/stat-index.json` and `cache/ast/` still held them. Pruned both plus the dead `graph.json` nodes, rebuilt: 6024 → 5152 nodes, 0 references to missing files.
- **Wiki drift closed:** [[systems/EnergyReservoirs]] still said `ReservoirBars` "exists but unused" (deleted 4.8 R-2); [[concepts/HudGate]] still listed `TeamScoreGui` and `ReticleBuilder` as live (deleted chunk 0, `ba9891d`); [[systems/VisualEffects]] still cited `MAX_TIER = 4` / `MAX_IMPACT_EFFECT_IDS` (gone with `VfxBroadcastService`, chunk 7; T4 parked); [[systems/Tests]] + [[index]] said 7 suites / 40 tests (8 / 43 — Analytics + `settings_tests`). Everything else the identifier lint flagged was already marked historical.
- **`CLAUDE.md` § Project Structure** rewritten — it still listed Firearm, Loadout, CameraManager/LocomotionManager, `StarterPack/Spelling Staff`, BossAdapter, `server/Vfx/VfxBroadcastService` and a Melee suite.
- **Trackers:** BRA.1–19 (Phase 1–4 build tasks + the 4.8 H-1 decouple) were still open months after shipping — all closed as done.
- **Studio junk, left for the user (all `.rbxl` content, nothing references it):** `ReplicatedStorage.Spawner` (Loadout pedestal/spawner meshes), `ReplicatedStorage.AnimationCarousel` + `ServerStorage.AnimationCarousel`/`AnimationCarouselData` (plugin dev copies), `ServerStorage.RBX_ANIMSAVES` (TPS-era animation editor saves), `Shared.Weapon.ViewModels` (already noted on [[systems/Weapon]]), `Shared.LocomotionAnimations` (also dead on disk — 2 meta files, no code reads it), `Packages/Promise` (wally dep, zero requires). `Shared.Weapon.Scripts/Effects` (empty, untracked dirs) removed from disk and Studio.
- **Also observed:** `rojo serve` died silently mid-session (no crash line in its log) — restarted detached; the Studio plugin needs a manual reconnect. Refactor plan chunks 0–12 all read `done` with commit hashes; working tree was clean and fully pushed before this pass.
